...
describe("Block rendering", () => {
  test("do values setups immediately while processing new value from scheme", () => {
    ...
    expect((fn.mock.calls[0] as any)[0]).toEqual({
...
describe("Block updating via function", () => {
  test("Block update function takes current value", () => {
    ...
    expect((updaterFunc.mock.calls[0] as any)[0]).toBeUndefined();
    expect((updaterFunc.mock.calls[1] as any)[0]).toEqual(
      expect.objectContaining({
        a: 1,
      })
    );
    expect((updaterFunc.mock.calls[2] as any)[0]).toEqual(
      expect.objectContaining({
        a: 33,
      })
    );
...
describe("Block instance options", () => {
  test("Block handleUpdate works well", () => {
    ...
    expect((fn.mock.calls[0] as any)[0]).toEqual({ a: 2, b: 2 });
...
describe("Block handler", () => {
  ...
  test("Block handler sync update works properly", () => {
    ...
    expect((fn.mock.calls[0] as any)[0]).toEqual({ a: 4, b: 100 });
    ...
    return new Promise<void>((resolve) => {
      const fn = jest.fn(async (value, update) => {
        ...
        if (value.a === 3) {
          ...
          expect((fn.mock.calls[0] as any)[2]).toBe(undefined); // oldValue

          expect(fn.mock.calls[1][0]).toEqual({ a: 1, l: true });
          expect((fn.mock.calls[1] as any)[2]).toEqual({ a: 1, l: false });

          expect(fn.mock.calls[2][0]).toEqual({ a: 2, l: true });
          expect((fn.mock.calls[2] as any)[2]).toEqual({ a: 1, l: true });

          expect(fn.mock.calls[3][0]).toEqual({ a: 3, l: true });
          expect((fn.mock.calls[3] as any)[2]).toEqual({ a: 2, l: true });
  ...
  test("Nested block handler works properly", () => {
    ...
    expect(fn.mock.calls.length).toBe(3);
    expect((fn.mock.calls[2] as any)[0]).toEqual({ a: 3, b: 2 }); // newValue
    expect((fn.mock.calls[2] as any)[2]).toEqual({ a: 1, b: 2 }); // oldValue
  });
